...
async def start_labjack_sample(queue):
    try:
        ...
        print(
            f"{stream_config.get('handle')} - Received stream_config: {stream_config}"
        )
        print(
            f"{stream_config.get('handle')} - Received channel_details: {channel_details}"
        )
        ...
        actual_scan_rate = await asyncio.to_thread(
            ljm.eStreamStart,
            handle,
            int(scan_rate / num_addresses),
            num_addresses,
            aScanList,
            scan_rate,
        )
        print(
            f"{handle} - Stream started with an actual scan rate of {actual_scan_rate} Hz."
        )
...
async def set_labjack_config(
    ...
    if not channels or not isinstance(channels, list):
        raise ValueError(
            "Invalid channels. Provide a non-empty list of channel names (e.g., ['AIN0', 'AIN1'])."
        )
    ...
    try:
        ...
        aScanList = await asyncio.to_thread(
            ljm.namesToAddresses, num_addresses, aScanListNames
        )
...
async def configure_each_labjack(serial_number, config):
    try:
        ...
        gain = config["gain"]
        settling_time = config.get(
            "STREAM_SETTLING_US", 0
        )  # Default to 0 if not provided
        resolution_index = config.get(
                ...
                async for update in watcher:
                    ...
                    try:
                        ...
                        if serial_number in queues:
                            for attempt in range(max_labjack_config_retries):
                                stream_config, channel_details = (
                                    await configure_each_labjack(
                                        serial_number, active_configs[key_name]
                                    )
                                )
                                if (
                                    stream_config is not None
                                    and stream_config.get("handle") is not None
                                ):
                                    print(
                                        "LabJack configuration obtained successfully."
                                    )
                                    print("stream_config : ", stream_config)
                                    print("Channel Details : ", channel_details)
                                    print(
                                        f"Starting labjack_sample for serial number {serial_number}"
                                    )
                                    queue = queues[serial_number]
                                    queue.put_nowait((stream_config, channel_details))
                                    tasks[serial_number] = asyncio.create_task(
                                        start_labjack_sample(queue)
                                    )
                                    break
                                else:
                                    print(
                                        f"Failed to configure LabJack for serial {serial_number} (attempt {attempt + 1}). Retrying..."
                                    )
                                    await asyncio.sleep(
                                        1
                                    )  # Adjust the delay between retries as needed
                        else:
                            print(
                        ...
                        if attempt == max_labjack_config_retries - 1:
                            print(
                                f"Failed to configure LabJack for serial {serial_number} after {max_labjack_config_retries} attempts. Skipping."
                            )
