...
connection_type_map = {0: "ANY", 1: "USB", 2: "TCP", 3: "Ethernet", 4: "WiFi"}

# Mapping for device types
device_type_map = {0: "ANY", 4: "T4", 7: "T7", 8: "T8", 84: "T-SERIES", 200: "DIGIT"}
...
config_schema = {
    "type": "object",
    "properties": {
        "scan_rate": {"type": "integer", "minimum": 1, "maximum": 40000},
        "gain": {
            "type": "number",
        ...
        "channels_enabled": {
            ...
            "patternProperties": {
                "^[0-9]+$": {
                    "type": "object",
                    "properties": {
                        "type": {"type": "string", "enum": ["SCS", "PC", "SG"]},
                        "name": {"type": "string"},
                        "raw_data": {"type": "string", "enum": ["volt", "cal"]},
                        "report_unit": {
                            "type": "string",
                            "enum": [
                                "V",
                                "mV",
                                "muV",  # For raw_data: "volt"
                                "m",
                                "mm",
                                "mum",  # For raw_data: "cal", type: "SCS"
                                "Pa",
                                "KPa",  # For raw_data: "cal", type: "PC"
...
async def check_buffer_and_prepare_publish(queue):
    ...
    file_lock = FileLock(
        f"all_data_{serial_number}.lock"
    )  # Create a file lock for the current channel
    try:
        while True:
            # if os.path.exists(f"{channel_name}_data.dat") and os.path.getsize(f"{channel_name}_data.dat") > 0:
            if (
                os.path.exists(f"all_data_{serial_number}.dat")
                and os.path.getsize(f"all_data_{serial_number}.dat") > 0
            ):
                print(
                    f"File all_data_{serial_number}.dat exists, starting to process data..."
                )
                ...
                if all_data_samples:
                    ...
                    for data_sample in all_data_samples:
                        ...
                        if is_first_sample:
                            each_channel_start_timestamp = [
                                start_timestamp
                                + datetime.timedelta(seconds=sample_idx / scan_rate)
                                for sample_idx in range(num_addresses)
                            ]
                            per_channel_time_delta = datetime.timedelta(
                                seconds=num_addresses / scan_rate
                            )
                        ...
                        for j, channel in enumerate(channel_details.values()):
                            ...
                            channel_data[channel_name].extend(
                                each_channel_data_samples
                            ...
                            while len(channel_data[channel_name]) >= nats_stream_rate:
                                ...
                                batch_data = channel_data[channel_name][
                                    :nats_stream_rate
                                ]
                                start_time_current_batch = each_channel_start_timestamp[
                                    j
                                ]
                                each_channel_start_timestamp[j] = (
                                    each_channel_start_timestamp[j]
                                    + (nats_stream_rate - 1) * per_channel_time_delta
                                )
                                channel_data[channel_name] = channel_data[channel_name][
                                    nats_stream_rate:
                                ]
                                ...
                                payload_info = {
                                    "start_timestamp": start_time_current_batch.isoformat(),
                                ...
                                await nats_publish(
                                    nats_topic, compressed_data, payload_info
                                )
                ...
                await asyncio.sleep(
                    0.5
                )  # added to ensure other labjack tasks start/continue if no file exists
...
async def monitor_bucket():
    ...
    try:
        ...
        )  # ljm.constants.ctANY  # Search for any connection type

        # Get connected LabJack devices
        num_found, device_types, connection_types, serial_numbers, ip_addresses = (
            ljm.listAll(device_type, connection_type)
        )
        ...
        for i in range(num_found):
            ...
            device_type_name = device_type_map.get(
                device_types[i], f"Unknown ({device_types[i]})"
            )
            connection_type_name = connection_type_map.get(
                connection_types[i], f"Unknown ({connection_types[i]})"
            )
            ...
            print(
                f"Published active LabJacks to {subject}: {json.dumps(labjacks, indent=2)}"
            )
            ...
            try:
                ...
                print(
                    f"Key '{key_name}' already exists with value: {existing_value.value.decode()}"
                )
        ...
        if num_found:
            ...
            while True:  # Keep watching indefinitely
                watcher = await kv.watch(
                    "labjackd.config.*"
                )  # Await to get the async iterator
                async for update in watcher:
                    ...
                    try:
                        ...
                        print(
                            f"Key '{key_name}' updated to revision {update.revision}: {key_value}"
                        )

                        # Check if the key is empty
                        if (
                            not key_value.strip() or key_value == "{}"
                        ):  # Empty or blank JSON
                        ...
                        if serial_number in tasks:
                            print(
                                f"Task for serial {serial_number} was already running"
                            )
                            task = tasks[serial_number]
                            if not task.done():
                                print(
                                    f"Cancelling the existing task for serial {serial_number}"
                                )
                                task.cancel()
                                try:
                                    ...
                                    print(
                                        f"Task for serial {serial_number} was cancelled"
                                    )
                            # Remove the task from the dictionary
                            del tasks[serial_number]

                        if serial_number in queues:
                            stream_config, channel_details = (
                                await get_each_labjack_config(
                                    serial_number, active_configs[key_name]
                                )
                            )
                            if stream_config is not None:
                                ...
                                tasks[serial_number] = asyncio.create_task(
                                    check_buffer_and_prepare_publish(queue)
                                )
                            else:
                                print(
                                    f"Failed to get configuration for LabJack with serial {serial_number})"
                                )
                        else:
                            print(
                                f"skipping for labjack {serial_number} as it is not detected"
                            )
                    except Exception as e:
                        print(f"An unexpected error occurred: {e}")
    except ErrNoServers:
        print(
            "Could not connect to NATS server. Ensure that the NATS server is running."
        )
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
    finally:
